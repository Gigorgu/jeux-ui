...
jQuery(document).ready(function ($) {
	...
	$('.carousel').toggleClass('.owl-carousel');
});
...
/*
var spoiler = {
	spoiler: document.querySelectorAll('[data-spoiler]'),
	data_spoiler: document.querySelector('[data-spoiler]').getAttribute('data-spoiler'),
	data_spoiler_text: document.querySelector('[data-spoiler]').getAttribute('data-spoiler-text'),
	data_spoiler_mobile: document.querySelector('[data-spoiler]').getAttribute('data-spoiler-mobile'),
	data_custom_spoiler: document.querySelector('[data-spoiler]').getAttribute('data-custom-spoiler'),
	ani: {
		animation: document.querySelector('[data-spoiler]').getAttribute('data-spoiler-animation')
	}
}

ropgkn();
function ropgkn() {
	$(spoiler.data_spoiler).toggleClass('spoiler-hidden')
	spoiler.spoiler.forEach((obj) => {
		if (spoiler.data_spoiler == '') {
			const objlow = edgjb(obj);
			obj.addEventListener('click', () => {
				$(objlow).toggleClass('show');
			})
		} else {

			obj.addEventListener('click', () => {
				$(spoiler.data_spoiler).toggleClass('show')
			})
			other_reee(obj);
		}
...
function other_reee(obj) {
	if (spoiler.ani.animation == "fadeIn") {
		document.querySelector('[data-spoiler]').classList.toggle('fadein');
		$(spoiler.data_spoiler).toggleClass('fadein');
	}
	if (spoiler.data_spoiler_mobile == 'true') {
		$(spoiler.data_spoiler).toggleClass('mobile-rt');
		$(obj).toggleClass('mobile-rt');
	} else {
		console.log('log err1');
	}
}

function edgjb(obj) {
	const spoiler_container = document.querySelector('.spoilert-rt');
	const objlow = document.createElement('div');
	$(objlow).toggleClass('spoiler-hidden');
	$(objlow).html(spoiler.data_spoiler_text);
	spoiler_container.appendChild(objlow);
	if (spoiler.ani.animation == "fadeIn") {
		document.querySelector('[data-spoiler]').classList.toggle('fadein');
		$(objlow).toggleClass('fadein');
	}
	if (spoiler.data_spoiler_mobile == 'true') {
		$(obj).toggleClass('mobile-rt');
		$(objlow).toggleClass('mobile-rt');
	} else {
		console.log('log err1');
	}
	return objlow;
